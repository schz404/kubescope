...
The project investigates the challenge of debugging and performing incident response/ forensic analysis on distroless container images—which lack standard shells and debugging utilities—without compromising the live state of the production environment.

## Architecture
This PoC utilizes:
1.  **Victim Workload:** A minimal, distroless Go application.
2.  **Forensic Agent:** A custom-built, hardened debug image (`kubescope`) containing industry-standard tools like `strace`, `tshark`, `lsof`, and `gdb`.
